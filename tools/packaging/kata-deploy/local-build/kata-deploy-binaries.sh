...
install_kernel_helper() {
	...
	export kernel_version="$(yq r $versions_yaml ${kernel_version_yaml_path})"
...
install_kernel_tdx_experimental() {
	local kernel_url="$(yq r $versions_yaml assets.kernel-tdx-experimental.url)"
...
install_qemu_helper() {
	...
	export qemu_repo="$(yq r $versions_yaml ${qemu_repo_yaml_path})"
	export qemu_version="$(yq r $versions_yaml ${qemu_version_yaml_path})"
	...
	"${builder}"
	tar xvf "${builddir}/kata-static-${qemu_name}.tar.gz" -C "${destdir}"
}
...
install_qemu_tdx_experimental() {
	...
	export qemu_tarball_name="kata-static-qemu-tdx-experimental.tar.gz"

	install_qemu_helper \
		"assets.hypervisor.qemu-tdx-experimental.url" \
		"assets.hypervisor.qemu-tdx-experimental.tag" \
		"qemu-tdx-experimental" \
